{
  ...
  flake.packages = let
    ...
    buildVMImage = {
      ...
      # Import the make-disk-image function
      makeDiskImage = import "${inputs.nixpkgs}/nixos/lib/make-disk-image.nix";

      # Base configuration for VM images
      baseConfig = {
        imports = [
          config
          {
    ...
    in
      makeDiskImage {
        ...
        configFile = pkgs.writeText "configuration.nix" ''
          ...
          # Edit the flake configuration instead
          {}
        '';
  ...
  in {
    ...
    vm-launcher-minimal = pkgs.writeShellScriptBin "launch-minimal-vm" ''
      ...
      VM_IMAGE="$(nix build --print-out-paths --no-link .#vm-image-minimal)"
    ...
    vm-launcher-desktop = pkgs.writeShellScriptBin "launch-desktop-vm" ''
      ...
      VM_IMAGE="$(nix build --print-out-paths --no-link .#vm-image-desktop)"
    ...
    vm-launcher-development = pkgs.writeShellScriptBin "launch-development-vm" ''
      ...
      VM_IMAGE="$(nix build --print-out-paths --no-link .#vm-image-development)"
    ...
    vm-manager = pkgs.writeShellScriptBin "vm-manager" ''
      ...
      case "$command" in
        build)
          ...
          nix build ".#vm-image-$vm_type" --print-build-logs
        ...
        list)
          ...
          for vm_type in minimal desktop development; do
            if nix build ".#vm-image-$vm_type" --dry-run >/dev/null 2>&1; then
              echo "  ✅ $vm_type"
            else
    ...
    vm-test-runner = pkgs.writeShellScriptBin "vm-test-runner" ''
      ...
      run_test() {
        ...
        if ! nix build ".#vm-image-$vm_type" --dry-run >/dev/null 2>&1; then
          ...
          nix build ".#vm-image-$vm_type" --quiet
        fi
      ...
      # Test 1: VM image builds
      for vm_type in minimal desktop development; do
        total_tests=$((total_tests + 1))
        if ! run_test "$vm_type" "Image Build" "nix build .#vm-image-$vm_type --quiet"; then
          failed_tests=$((failed_tests + 1))
        fi
      done

      # Test 2: VM launcher scripts build
      for vm_type in minimal desktop development; do
        total_tests=$((total_tests + 1))
        if ! run_test "$vm_type" "Launcher Build" "nix build .#vm-launcher-$vm_type --quiet"; then
          failed_tests=$((failed_tests + 1))
        fi
      done

      # Test 3: VM manager tool
      total_tests=$((total_tests + 1))
      if ! run_test "manager" "VM Manager Tool" "nix build .#vm-manager --quiet"; then
        failed_tests=$((failed_tests + 1))
      fi
